...
public class AppMapScheduler implements Job {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		System.out.println("App mapSchedule called by quartz");
		...
		try {
			...
			try {
				DBCollection PlayerColl = db.getCollection("Regular");
				DBCollection insertColl = db.getCollection("OneHourAvgForAppMap");

				BasicDBObject document = new BasicDBObject();
				DBObject match1 = new BasicDBObject("$match",
						new BasicDBObject("request_time", new BasicDBObject(
								"$gt", endTime)));
				DBObject group1 = new BasicDBObject("$group",
						new BasicDBObject("_id", "$state").append("TotalCount",
								new BasicDBObject("$sum", 1)).append(
								"TotalSum",
				...
				for (DBObject results1 : output1.results()) {
					...
					if (!stateCode.startsWith("INVA")) {
						...
						double TotalSum = Double.parseDouble(results1.get(
								"TotalSum").toString());
						double avgAppResponse = Double.parseDouble(formatter
								.format(TotalSum / TotalCount));

						subObj.put("AppTotalCount", TotalCount);
						subObj.put("AppTotalSum", TotalSum);
						subObj.put("OneHourAvg", avgAppResponse);

						System.out.println("AverageOfMap for : " + stateName
								+ "->" + stateCode + " ->" + avgAppResponse);
				...
				if (flagIfOutputNull) {
					...
					System.out.println("no data found:App Map  Schedular");
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (date2.compareTo(date1) < 0) {
				...
				DBCollection oneDayColl = db.getCollection("OneHourAvgForAppMap");
				DBCollection AppOneDayAvgForMap = db
				...
				for (String key : stateKet) {
					...
					double stateOneDayAvg = Double.parseDouble(formatter
							.format(sum / count));
					stateData.put("AppOneDayCount", count);
					stateData.put("AppOneDaySum", sum);
					stateData.put("AppOneDayAvg", stateOneDayAvg);
					oneDayInsertObj.put(key, stateData);
				}
				if (oneDayInsertObj.size() > 0) {
					oneDayInsertObj.put("time", nowTime);
					AppOneDayAvgForMap.insert(oneDayInsertObj);
	...
	public static void main(String[] args) throws Exception {
		AppMapScheduler j = new AppMapScheduler();
		j.execute(null);
